...
class TenantController extends Controller
{
    ...
    public function updateNumRoom(Request $request)
    {
        ...
        $title = 'xác nhận phòng điện, nước ';
        $content = 'xác nhận số điện , nước lúc vào trọ ';
        $sender_id = $userId;
        $receiver = $room->latest_tenant->tenant_users;
        $noti_type_id = 4;
        $tenant->water_num = $request->water_num;
        $tenant->elec_num = $request->elec_num;
        $tenant->save();

        DB::transaction(function () use ($title, $sender_id, $receiver, $content, $noti_type_id) {
            foreach ($receiver as $receiver_id) {
                Noti::insert([
                    'title' => $title,
                    'sender_id' => $sender_id,
